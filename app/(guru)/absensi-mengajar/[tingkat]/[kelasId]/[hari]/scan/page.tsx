...
export default function ScanQrKelasPage() {
  ...
  const handleScan = () => {
    ...
    // Simulasi scanning 1.2 detik lalu acak atau sukses (80% sukses)
    setTimeout(() => {
  ...
  const handleContinue = () => {
    // Navigasi ke halaman Selfie (Ambil Foto Mengajar)
    router.push(`/absensi-mengajar/${tingkat}/${kelasId}/${hari}/foto`);
  };

  return (
    <div className="flex flex-col min-h-screen bg-[#EEF2F7] px-6 pt-12 pb-10">
      {/* ── HEADER RESPONSIONAL SESUAI FIGMA ── */}
      {scanState === "IDLE" || scanState === "SCANNING" ? (
        <PageHeader
          backHref={`/absensi-mengajar/${tingkat}/${kelasId}/${hari}`}
          title="Scan QR Kelas"
      ...
      {/* ── MODE IDLE / SCANNING: Kamera Real + Area QR Box ── */}
      {(scanState === "IDLE" || scanState === "SCANNING") && (
        <>
